...
public class XoninUniqueTiger : CharacterState
{
    public XoninUniqueTiger(CharacterStateMachine currentContext, CharacterStateFactory CharacterStateFactory) : base(currentContext, CharacterStateFactory)
    {
        ...
        this.animationName = this.character.characterName + "_UniqueTiger";
    }
    ...
    public override bool OnHurt() {
        if (this.character.enemy.currentState.moveType != MoveType.AIR && this.stateTime > 6  && this.stateTime <= 15) {
            this.SwitchState((this.states as XoninStateFactory).UniqueTigerCounter());
            return false;
        }
        return true;
    }
